		...
		<%String detailedURL= request.getParameter("userLogin");
			String[] parameters=detailedURL.split(",");
		%>
		 <div class="header">
				<h3>Send Replay to ADR</h3>
			  </div>
			  <form method="post" action="saveReplay.jsp" id="registartionForm">
					<div class="input-group">
				 <div class="input-group">
				 <label>To</label>
					<input type="text" name="userLogin" placeholder=<%= parameters[0]%> value=<%= parameters[0]%> readonly>
						</div>
						<div class="input-group">
						<label>Observed ADR for Medicine</label>
						<input type="text" name="medicine" placeholder="medicine" value=<%= parameters[2]%> readonly>
						</div>
						<div class="input-group">
						<input type="text" name="adrid" placeholder="ADR ID" value=<%= parameters[1]%> hidden>
						</div>
						<div class="input-group">
						<label>Message</label>
						<textarea name="replay" rows="4" cols="40" onfocus="this.value=''">Enter reply here</textarea>
						</div>
						</br>
					 <div class="input-group">
					<input id="submit" type="submit" value="Send Replay" class="btn">
					</div>
					  </div>
					</form>	
		<p>&nbsp</p>
