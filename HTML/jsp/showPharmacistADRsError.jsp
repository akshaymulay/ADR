	...
	<%@page import="java.sql.*,java.util.*"%>
		...
		<head>
				<meta charset="utf-8" />
		<title>Pharmacist Raised ADRs</title>
		...
		<div align="center">
		  <!-- /. main-header -->
		  <span class="btn-success page-description">ADRs Raised By Pharmacists</span>
	    </div>
		<div>
		<table class="table" border="2">
		<tr>
		<th class="btn-info">Sr.Id</th>
		<th class="btn-info">Pharmacist Name</th>
		<th class="btn-info">Pharmacist Contact No</th>
		<th class="btn-info">Pharmacist Email</th>
		...
		<th class="btn-info">Problem With Another Product Of Same Company</th>
		...
		prop.load(fis);
		 ...
		 String sql = "select adrid,pharmacistName,pharmacistContact ,pharmacistEmail ,medicine ,productManufacture ,manufacturingDate ,expirationDate ,noPrescription ,personpDetails,anotherProduct,problemOccured,reactionBycustomer,sideEffect,userLogin from pharmacistADRs";
 ...
 while(resultSet.next()){ %>
		 ...
		 <TD class="list-group-item-success"> <%= resultSet.getString(7) %></TD>
		 <TD class="list-group-item-success"> <%= resultSet.getString(8) %></TD>
		 <TD class="list-group-item-success"> <%= resultSet.getString(9) %></TD>
		 <TD class="list-group-item-success"> <%= resultSet.getString(10) %></TD>
		 <TD class="list-group-item-success" colspan=1> <%= resultSet.getString(11) %></TD>
		 <TD class="list-group-item-success" colspan=2> <%= resultSet.getString(12) %></TD>
		 <TD class="list-group-item-success" > <%= resultSet.getString(13) %></TD>
		  <%
		 if(!resultSet.getString(14).equals("guest")){%>
			 		 <TD class="btn-warning"> <a href="sendReplay.jsp?userLogin=<%=resultSet.getString(14)%>,<%=resultSet.getString(1)%>,<%=resultSet.getString(4)%>"> Send Replay
		 </a>
		 <%}
		 else{%>
		...
		e.printStackTrace();
		response.sendRedirect("showPharmacistADRsError.jsp");
	} 	 
		 %>
							 </TD>
		 		 </tr>
